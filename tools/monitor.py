...
def _total_sections() -> int:
    try:
        sys.path.insert(0, str(Path(__file__).resolve().parent.parent / "legacy"))  # deep_research is legacy/ post-reorg
    ...
    while True:
        ...
        if p:
            ...
            if line != last:
                ...
                last = line
            if p["written"] >= TOTAL:
                print("\nDONE! Check output/runs/book/book.pdf")
                r = get_report()
                if r:
