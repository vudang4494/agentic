...
> **File 2 phần:**
> **A · CẤU TRÚC CHUẨN của PRODUCT** (authoritative — đồng bộ với `CLAUDE.md mục 3` + code `research/*.py`).
...
> Chi tiết code-level (12 sub-stage · file:line · ngưỡng thật) → **`CLAUDE.md mục 3` + `docs/RULES.md`** (nguồn sự thật vận hành). File này = **bản đồ kiến trúc chuẩn** (tầng cao).
