...
> **Vai trò:** Đây là **nguồn ngưỡng chuẩn duy nhất**. Mọi doc khác defer về đây. Phân biệt rõ **OPERATIVE** (code enforce thật) vs **TARGET** (aspirational, chưa enforce). Mục tiêu academic chi tiết → `eval/PRODUCT_QUALITY_CRITERIA.md` (không lặp ở đây).
...
> → **Gate cứng: P0a (~0.40, PRE-writer) + StageD word-count + G2 cite_prec≥0.45 (giờ là gate SỐNG, cho qua section faithful).** Chi tiết: `docs/plan.md` mục Upgrade.
...
- G1 **P0a** domain ≈0.40 HARD ✅ (pre-writer, gate cứng SỐNG) · G3 **grounding = log-only/advisory** (P0: bỏ khỏi gate) · G4 **topic** ✅ ENFORCED (gate_ok + StageE best-topic) · G2 **citation integrity** ✅ GATE SỐNG (P0+P0-2b; cite_precision đo thật, judge soften → faithful prose ≥0.45 clean-accept, weak floor) · G5a cross-ref `:630` · G5b numeric-ref hint (soft). **→ P0+P0-2b done: G2 discriminate thật (GOOD 0.72 vs BAD 0.18/0.20), faithful section `quality="ok"`. Roadmap kế = P1 (`docs/plan.md`).**
...
- **Invariant:** mọi HARD message in ngưỡng thật (biến, không literal); 1 grounding + 1 topic definition; 1 embed model; **mọi judge = model LOCAL (gemma/HHEM/bge), KHÔNG Claude/external lúc runtime.** Unit test: `python3 eval/test_verify_optim.py`.
