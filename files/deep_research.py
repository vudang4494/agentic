...
SYS = (
    ...
    "   (f) NEVER cite 'Tavily', 'DuckDuckGo', 'DDG', 'Brave', 'Google Search', or any other "
    "search-provider name as if it were a research author. Tavily/DDG/Brave are retrieval "
    "tools, not researchers. Use the actual author/lab name from the EVIDENCE entry, or use "
    "the source title in italics if no author is available. Bad: 'Tavily (2024) showed...'. "
    "Good: 'Wei et al. (2022) showed...' or '*Survey of LLM Eval* shows...'.\n"
    "   (g) NEVER emit a literal `[N]` placeholder. Either pick a real number from the EVIDENCE "
    "block or remove the citation entirely. The string `[N]` in your output is a bug.\n"
    "6. When no EVIDENCE block is present, fall back to inline author-year style "
    "`(Vaswani et al., 2017)` for well-known canonical references only -- still never use a "
    "search-provider name as the author.\n"
