...
# For continuous tailing, use: ./run.sh watch  (or python3 tools/monitor.py)
set -e
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
cd "$SCRIPT_DIR"

STATE="output/runs/book/state.json"
STDOUT_LOG="output/runs/book/pipeline.stdout.log"
...
echo "    pkill -f legacy/deep_research.py   (kill)"
