...
Usage:
    python3 eval/run_eval.py --topic transformer
    python3 eval/run_eval.py --topic transformer --skip-pipeline  # re-score archived run

Workflow:
    1. Load eval/topics/<name>.yaml (gold standard)
    2. Run deep_research.run() with the topic's n_chapters / n_passes
    3. Read output/runs/eval_<name>_<ts>/state.json
    4. Compute per-section + aggregate + pass/fail metrics
    5. Write eval/reports/eval_<name>_<ts>.{json,md}
"""
...
ROOT = HERE.parent  # eval/ -> repo root post-reorg
sys.path.insert(0, str(ROOT))
sys.path.insert(0, str(ROOT / "legacy"))  # deep_research.run() lives in legacy/
...
def _load_state(out_name: str) -> dict:
    state_path = ROOT / "output" / "runs" / out_name / "state.json"
...
def main() -> int:
    ...
    p.add_argument("--topic", required=True,
                   help="topic slug under eval/topics/<slug>.yaml")
    ...
    if not args.skip_pipeline:
        ...
        state_path = Path(args.state) if args.state else ROOT / "output" / "runs" / out_name / "state.json"
        with open(state_path) as f:
            state = json.load(f)
