...
- **DOCTRINE (user affirmed, BẤT BIẾN):** Agentic Deep Research lên chất lượng ở tầng **orchestration/inference** (retrieval/verify/revise-loop/prompt/evidence-select), **KHÔNG train model & KHÔNG build dataset** (giữ topic-agnostic, prompt-robust, auditable). Codify: `CLAUDE.md mục 2/mục 6.9`, `RULES #8 + Product-FAIL #7`, `docs/plan.md P1.5`.
...
- **Thay đổi:** clean toàn bộ docs (RULES/CLAUDE/GLOSSARY/README + memory) về đúng "verify post-writer INERT, P0a là gate sống" + viết mục Upgrade roadmap vào `docs/plan.md`.
...
- **Phát hiện chính:**
  ...
  - **Doc drift nặng:** CLAUDE/short-memory ghi v3.4/v3.5, "32 section/19K từ/4 canonical", module ở `files/*.py`, P0a=0.60/0.80 — ĐỀU SAI. Thật: deep_research_v3.py + `research/*.py`, 280 section, 12 canonical, P0a≈0.40, grounding accept 0.70, topic 0.50.
...
- **Thay đổi đã làm:**
  - Viết lại `CLAUDE.md` (pipeline 12-stage thật + 8 guardrails + ngưỡng code), `docs/RULES.md` (bảng OPERATIVE vs TARGET), `short-memory.md` (snapshot v36), `long-memory.md` (de-dup entry 06-08).
